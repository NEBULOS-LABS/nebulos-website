...
console.log('[LaserFlow] module loaded (tsx version)');
...
uniform float uFade;
uniform float uCardTop;
uniform float uCardRight;
uniform float uCardCenter;
...
/* ── Brand Palette (accretion-disk coloring from redesign) ── */
...
vec3 brandPalette(float t) {
  t = fract(t);
  if (t < 0.60) return mix(BP_CYAN, BP_PURPLE, t / 0.60);
  else if (t < 0.80) return mix(BP_PURPLE, BP_MAGENTA, (t - 0.60) / 0.20);
  else return mix(BP_MAGENTA, BP_CYAN, (t - 0.80) / 0.20);
}

/* ── HDR Tone Mapping (from redesign) ── */
...
float vWisps(vec2 uv,float topF){
    ...
    for(int s=0;s<2;++s){
        ...
        for(int i=0;i<10;++i){
            ...
            float dx=abs(uv.x-xc),lat=1.1-smoothstep(W_HALF,W_HALF+W_AA,dx),amp=exp(-off*W_SIDE_DECAY);
    ...
    float span=smoothstep(-50.0,0.0,y)*(1.0-smoothstep(R_V-6.0,R_V,y));
    return uWIntensity*sum*topF*bGain*span;
}

vec2 getMappedUV(vec2 p) {
    float R = max(uRadius, 0.001);
    float L1 = R * PI * 0.5;
    float hw = max(0.0, uCardRight - uCardCenter - 2.0 * R);
    float L2 = L1 + hw;

    vec2 C1 = vec2(uCardCenter + R, uCardTop + R);
    vec2 C2 = vec2(uCardRight - R, uCardTop - R);

    vec2 mapped = vec2(0.0);

    if (p.y >= C1.y && p.x <= C1.x) {
        // Zone 1: Vertical approach (above card, left of corner arc)
        mapped.x = p.x - uCardCenter;
        mapped.y = C1.y - p.y;
    } else if (p.y <= C2.y && p.x >= C2.x) {
        // Zone 2: Vertical descent (right side)
        mapped.x = p.x - uCardRight;
        mapped.y = L2 + R * PI * 0.5 + (C2.y - p.y);
    } else if (p.x <= C1.x && p.y <= C1.y) {
        // Zone 3: Inner arc (90 deg around top-left corner)
        vec2 d = p - C1;
        float angle = atan(-d.y, -d.x);
        mapped.x = R - length(d);
        mapped.y = R * clamp(angle, 0.0, PI*0.5);
    } else if (p.x >= C2.x && p.y >= C2.y) {
        // Zone 4: Outer arc (90 deg around top-right)
        vec2 d = p - C2;
        float angle = atan(d.x, d.y);
        mapped.x = length(d) - R;
        mapped.y = L2 + R * clamp(angle, 0.0, PI*0.5);
    } else {
        // Zone 5: Horizontal connection along card top
        mapped.x = p.y - uCardTop;
        mapped.y = L1 + (p.x - C1.x);
    }
    return mapped;
}
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    vec2 mapped = getMappedUV(uvc);

    float b=0.0;
    float mappedY = mapped.y;
    float tLine = mapped.y + 250.0;

    float s = clamp(tLine / (R_V * uVLenFactor), 0.0, 1.0);
    float baseEnv = pow(1.0 - s, 0.6);
    float cap = 1.0 - smoothstep(TOP_FADE_START, 1.0, s);
    baseEnv *= pow(cap, TOP_FADE_EXP);

    float ph = s / max(FLOW_PERIOD, EPS) + uFlowTime * uFlowSpeed;
    float fl = pow(tri01(ph), FLOW_SHARPNESS);
    float env = baseEnv * mix(1.0 - uFlowStrength, 1.0, fl);

    float m = pow(smoothstep(FLARE_HEIGHT, 0.0, tLine), FLARE_EXP);
    float wx = 1.0 + FLARE_AMOUNT * m;

    float f = env * uFalloffStart;
    float dd = (mapped.x * mapped.x) / (wx * wx);

    float coreD = exp(-dd / (f * f * 0.8 + EPS));
    float bloomD = (f * f) / (dd * 0.15 + f * f + EPS);

    float r = coreD * 0.7 + bloomD * 0.4;
    b = env * r * 2.5;

    float sPix=clamp(mappedY/R_V,0.0,1.0),topA=pow(1.0-smoothstep(TOP_FADE_START,1.0,sPix),TOP_FADE_EXP);
    float L=b*topA;
    float w=vWisps(mapped, topA);
    float fog=0.0;
#if FOG_ON
    ...
    float yP=1.0-smoothstep(HFOG_Y_RADIUS,HFOG_Y_RADIUS+HFOG_Y_SOFT,abs(mappedY));
    ...
    float totalLen = R_V * uVLenFactor;

    /* ── Spatial phase along the beam path (arc-length normalized) ── */
    float bPhase = clamp((mappedY + 15.0) / max(totalLen * 0.6, EPS), 0.0, 1.0) * 0.9;

    /* ── Time-animated phase shift (accretion-disk style) ── */
    ...
    /* Beam: 5 samples at staggered phases */
    ...
    /* Fog: 3 samples phase-shifted from beam */
    ...
    /* DEBUG: show card geometry markers */
    float dC = abs(uvc.x - uCardCenter);
    float dT = abs(uvc.y - uCardTop);
    float dR = abs(uvc.x - uCardRight);
    if (dC < 1.5) { col = vec3(1.0, 0.0, 0.0); alpha = 1.0; }  /* red = uCardCenter */
    if (dT < 1.5) { col = vec3(0.0, 1.0, 0.0); alpha = 1.0; }  /* green = uCardTop */
    if (dR < 1.5) { col = vec3(0.0, 0.0, 1.0); alpha = 1.0; }  /* blue = uCardRight */
...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uCardTop: { value: cardTop },
      uCardRight: { value: cardRight },
      uCardCenter: { value: cardCenter },
  ...
  useEffect(() => {
    ...
    uniforms.uFade.value = hasFadedRef.current ? 1 : 0;
    uniforms.uCardTop.value = cardTop;
    uniforms.uCardRight.value = cardRight;
    uniforms.uCardCenter.value = cardCenter;
    uniforms.uRadius.value = cornerRadius;

    if (cardTop !== 0 || cardCenter !== 0) {
      console.log('[LaserFlow] uniforms set:', { uCardTop: cardTop.toFixed(2), uCardRight: cardRight.toFixed(2), uCardCenter: cardCenter.toFixed(2), uRadius: cornerRadius.toFixed(2) });
    }
  }, [
    wispDensity,
    mouseTiltStrength,
